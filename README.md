...
**从科技趋势、行业舆情、热点追踪，到旅行规划、消费决策、日常问题，为你的 OpenClaw 装上更强的搜索能力。**
...
它让你可以直接借力大厂已经打磨好的搜索入口与搜索框架，把科技趋势、行业舆情、热点追踪、旅行规划、消费决策、日常问题统一接进自己的 Agent 与工作流。
...
> **把多平台 AI 搜索能力接成一个入口，直接借力大厂已经打磨好的搜索入口与搜索框架。**
...
AI Search Hub 不是传统 SDK，而是一个 Skill。

把这个仓库交给支持 Skill 的 Agent 或工具，例如 `OpenClaw`、`Claude Code`、`Codex`、`Cursor`，让它们自行安装或挂载这个 Skill 即可。
